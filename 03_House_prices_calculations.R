...
# 4.1 Create dataset for plot
Latest_data <- Price_changel %>% 
               mutate(Max_Date = max(datef)) %>% 
               mutate(Latest_month = ifelse(datef==Max_Date,1,0))

Min_date <- min(Latest_data$datef)
Max_date <- max(Latest_data$datef)
Min_date
Max_date

# 4.2 Create plot using previous data set
MoM_perc_change <-ggplot(data = Price_changel, aes( x = datef, y = percent, color = metric )) + 
  geom_line() +
  labs(title ="UK Average house prices into reverse from last year all time high.2005-2023 period",
...
ggsave(paste0("plots/17_UK_MoM_and_YoY_percent_price_change_JAN25_DEC23.jpeg"),width = 30, height = 20, dpi = 150, units = "cm")
